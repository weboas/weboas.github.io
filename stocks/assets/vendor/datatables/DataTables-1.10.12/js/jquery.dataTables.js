...
{if(!DataTable.__browser){var browser={};DataTable.__browser=browser;var n=$('<div/>').css({position:'fixed',top:0,left:0,height:1,width:1,overflow:'hidden'}).append($('<div/>').css({position:'absolute',top:1,left:1,width:100,overflow:'scroll'}).append($('<div/>').css({width:'100%',height:10}))).appendTo('body');var outer=n.children();var inner=outer.children();if(outer.length>0)
browser.barWidth=outer[0].offsetWidth-outer[0].clientWidth;else
browser.barWidth=0;if(inner.length>0)
browser.bScrollOversize=inner[0].offsetWidth===100&&outer[0].clientWidth!==100;else
browser.bScrollOversize=0;browser.bScrollbarLeft=Math.round((inner.length?inner.offset().left:0))!==1;if(n.length){browser.bBounding=n[0].getBoundingClientRect().width?true:false;n.remove();}
else
browser.bBounding=false;}
...
{$(n).bind('click.DT',oData,function(e){if($(e.target).is('i')||$(e.target).is('div')){return true;}else{n.blur();fn(e);}}).bind('keypress.DT',oData,function(e){if(e.which===13){e.preventDefault();fn(e);}}).bind('selectstart.DT',function(){return false;});}
